...
int main(int argc, char *argv[])
{
	...
	skeletal_v_sql,cut_v_update_sql;
...
cut_v_update_sql = "UPDATE %s \
SET component_%s = -ABS(%s.component_%s) FROM %s \
WHERE (%s.source = %s.id OR %s.target = %s.id) \
AND ABS(%s.component_%s) != 1 AND %s.component_%s < 0";
...
try {
	...
	if (C.is_open()) {
		...
		for (int i = 1; i <= num_levels; ++i) {
            ...
            // Updating the vertices connected to skeleton with negative component id
			pqxx::work U2(C);
