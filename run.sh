...
time mpirun -n 8 time ./g_b $dbname $num_levels $exponent > "data/$1_levels_out.txt"
...
psql -U postgres -d $dbname -t -A -F"," -c "select id, source, target, cost, x1, y1, x2, y2, level from cleaned_ways" > "./data/$1_geom.csv"
echo '2. Computing connections to make skeleton strongly connected at each level'
time ./c_c "$1_geom" $num_levels
echo '3. Computing paths between the above calculated connections'
time mpirun -n 8 time ./p_c "$dbname_geom" "$dbname_geom_conns" > "data/$dbname_pedges_out.txt"
